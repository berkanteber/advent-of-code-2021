...
def solve(data: str) -> int:
    ...
    for number in numbers:
        to_compress = [True for board in remaining_boards]
        for i, board in enumerate(remaining_boards):
            if board.mark_number(number) and board.check_bingo():
                to_compress[i] = False
                last_result = board.calculate_score(number)
        remaining_boards = list(compress(remaining_boards, to_compress))
...
assert solve(example_data) == 1924

print(solve(data))  # 6256
